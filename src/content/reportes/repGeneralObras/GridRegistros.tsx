...
export const GridRegistros = () => {
  ...
  const ImprimirReporte = async () => {
    if (ImprimiendoSlot) {
      ...
      //convertir MultiselectObras a string
      let obras = '';
      MultiselectObras.forEach((element) => {
        obras += element + ',';
      });

      let encrypted = encrypt(`${Empresa.id.toString()},${obras}`);

      const url = `${APP.URL_API}api/reportes/RepGeneralObras/${encrypted}`;
  ...
  const getData = async () => {
    const valoresForm = {
      empresa: Empresa,
      obras: MultiselectObras
    };

    if (BuscarDataSlot) {
      dispatch(setIsLoading(true));
      try {
        const response = await axios.post(`/api/obras/getrepgeneralobras`, {
          ...valoresForm
        });
  ...
  const columns = [
    {
      name: 'Clave',
      cell: (row) => <Box>{row.clave}</Box>,
      sortable: true,
      width: '15%'
    },
    {
      name: 'Nombre',
      cell: (row) => <Box>{row.nombre}</Box>,
      sortable: true,
      width: '25%'
    },
    {
      name: 'Fecha Inicio',
      cell: (item) => (
        <FechaConFormato fecha={item.fechaInicio} formato={'dd/MM/yyyy'} />
      ),
      sortable: true,
      width: '10%'
    },
    {
      name: 'Fecha fin',
      cell: (item) => (
        <FechaConFormato fecha={item.fechaFin} formato={'dd/MM/yyyy'} />
      ),
      sortable: true,
      width: '10%'
    },
    {
      ...
      cell: (row) => (
        <>
          {row.presupuesto.toLocaleString('es-MX', {
            style: 'currency',
            currency: 'MXN'
          })}
        </>
    ...
    },
    {
      ...
      cell: (row) => (
        <>
          {row.gastos.toLocaleString('es-MX', {
            style: 'currency',
            currency: 'MXN'
          })}
        </>
