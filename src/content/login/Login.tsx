...
const Login = () => {
  ...
  const [productoSeleccionado, setProductoSeleccionado] = useState<ProductosEmpresa>({
    id_producto: 0,
    desc_producto: ''
  });
  const [sucursalSeleccionada, setSucursalSeleccionada] = useState<sucursalInterface>({
    clave_sucursal: 0,
    nombre: '',
    telefono: '',
    domicilio: '',
  ...
  async function Login(credenciales: User) {
    try {
     ...
     if(response.status === 200){
      ...
      const productos = await getProductosByData(response.data.result as ProductosEmpresa[]);
  ...
  async function getSucursales() {
    ...
    if(response.data.isSuccess == true){
      ...
      const sucursales = await getAuth2(response.data.result as sucursalInterface[]);
      ...
      setSucursales(sucursales.map((sucursal) => {
        ...
        if(sucursales.length == 1){
          ...
          construirToken(sucursales[0].clave_sucursal, productoSeleccionado.id_producto,sucursales[0]);
  ...
  async function construirToken(id_sucursal: number , id_rol: number, sucursal: sucursalInterface){
...
const handleProductoChange = (event: SelectChangeEvent<number>) => {
  ...
  const selectedProduct = Productos.find(producto => producto.id_producto === selectedId);
...
const handleSucursalChange = (event: SelectChangeEvent<number>) => {
  const selectedId = event.target.value as number;
  const selected = Sucursales.find(producto => producto.clave_sucursal === selectedId);
    ...
    <><div>
      ...
      {Vista === 'Login' ?
        <FormularioLogin
          ...
          onSubmit={async (valores) => await Login(valores)} /> : <ForgotPassword />}
      <hr />
      <a href="/" onClick={ActionFP}>
        {Vista === 'Login' ? 'Olvidé mi contraseña' : 'Volver al Login'}
          ...
          onClick={async () => {
            await construirToken(sucursalSeleccionada.clave_sucursal, productoSeleccionado.id_producto, sucursalSeleccionada);
          }}
          style={{ marginRight: '10px' }}
          size='small'
          >
